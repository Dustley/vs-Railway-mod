...
public class ShipForceInducer implements ShipForcesInducer {

    private List<Force> forcesToApply = new ArrayList<>();

    public static ShipForceInducer getOrCreate(ServerShip ship) {
        ShipForceInducer inducer = ship.getAttachment(ShipForceInducer.class);
        if (inducer == null)  {
            inducer = new ShipForceInducer();
            ship.saveAttachment(ShipForceInducer.class, inducer);
        }
        return inducer;
    }
    ...
    public static enum ForceType {
        INVARIANT_LINEAR,INVARIANT_LINEAR_AT,ROTDEP_LINEAR,INVARIANT_ANGULAR,ROTDEP_ANGULAR;
    }
